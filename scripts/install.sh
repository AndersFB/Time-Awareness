...
if command -v apt-get >/dev/null 2>&1; then
  ...
  ask_proceed
  sudo apt-get update -qq || { echo " failed\n\n[ERROR] Failed to update package list" > /dev/tty; exit 1; }
  PKGS=(
    python3-gi
    libgtk-3-bin
  ...
  for pkg in "${PKGS[@]}"; do
    ...
    sudo apt-get install -y "$pkg" >/dev/null 2>&1 && progress_bar 10 || { echo " failed\n\n[ERROR] Failed to install $pkg" > /dev/tty; exit 1; }
  done
  ...
  sudo dnf config-manager --set-enabled crb && sudo dnf makecache >/dev/null 2>&1 && progress_bar 10 || { echo " failed\n\n[ERROR] Failed to enable CRB repository" > /dev/tty; exit 1; }
  PKGS=(
    python3-gobject
  ...
  for pkg in "${PKGS[@]}"; do
    echo -n "Installing $pkg "
    sudo dnf install -y "$pkg" >/dev/null 2>&1 && progress_bar 10 || { echo " failed\n\n[ERROR] Failed to install $pkg" > /dev/tty; exit 1; }
  done
else
...
if [ -d "$INSTALL_DIR" ]; then
  ...
  cd "$INSTALL_DIR" || { echo "\n[ERROR] Failed to change directory to $INSTALL_DIR" > /dev/tty; exit 1; }
  ...
  if command -v yum >/dev/null 2>&1 || command -v dnf >/dev/null 2>&1; then
    ...
    if [ "$EXT_STATUS" != "Yes" ]; then
      ...
      gnome-extensions install "$INSTALL_DIR/lib/appindicatorsupportrgcjonas.gmail.com.v60.shell-extension.zip" >/dev/null 2>&1 && progress_bar 10 || { echo " failed\n\n[ERROR] Failed to install appindicator GNOME extension" > /dev/tty; exit 1; }
      ...
      gnome-extensions enable "appindicatorsupport@rgcjonas.gmail.com" >/dev/null 2>&1 && progress_bar 5 || { echo " failed\n\n[ERROR] Failed to enable appindicator GNOME extension" > /dev/tty; }
    fi
  ...
  python3 -m venv .venv >/dev/null 2>&1 && progress_bar 10 || { echo " failed\n\n[ERROR] Failed to create Python virtual environment" > /dev/tty; exit 1; }
fi

echo
echo -n  "Step 3: Activate Python virtual environment and install Python dependencies."
ask_proceed
source .venv/bin/activate || { echo "\n[ERROR] Failed to activate Python virtual environment" > /dev/tty; exit 1; }
echo -n "Upgrading pip "
pip install --upgrade pip >/dev/null 2>&1 && progress_bar 5 || { echo " failed\n\n[ERROR] Failed to upgrade pip" > /dev/tty; exit 1; }
...
if ! dpkg -s libgirepository-2.0-dev &>/dev/null; then
  ...
  sed 's/^pygobject.*/pygobject==3.50.1/' requirements.txt > requirements_pinned.txt || { echo "\n[ERROR] Failed to pin PyGObject version" > /dev/tty; exit 1; }
...
pip install -U -r "$PIP_REQUIREMENTS_FILE" >/dev/null 2>&1 && progress_bar 10 || { echo " failed\n\n[ERROR] Failed to install Python dependencies" > /dev/tty; exit 1; }
...
gtk-launch time_awareness >/dev/null 2>&1 && progress_bar 5 || { echo " failed\n\n[ERROR] Failed to launch the app" > /dev/tty; exit 1; }
